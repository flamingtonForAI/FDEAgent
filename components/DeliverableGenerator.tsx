...
import { buildManifest } from '../utils/exportManifest';
...
const DeliverableGenerator: React.FC<DeliverableGeneratorProps> = ({
  ...
  const handleGenerate = async (type: DeliverableType) => {
    ...
    const config = deliverableConfigs.find(c => c.id === type);
    const format = config?.format || 'unknown';
    const qualityReport = runQualityCheck(project);
    const manifest = await buildManifest(project, format.toLowerCase(), qualityReport.score);

    let content = '';
    switch (type) {
      case 'api-spec': {
        const yamlContent = generateAPISpec(project);
        const parsed = YAML.parse(yamlContent);
        parsed._meta = manifest;
        content = YAML.stringify(parsed);
        break;
      }
      case 'data-model': {
        content = generateDataModel(project, i18nLang);
        content += `\n<!-- Generated by Ontology Architect v${manifest.generator.version} -->`;
        break;
      }
      case 'agent-tools': {
        const parsed = JSON.parse(generateAgentTools(project));
        const withMeta = { _meta: manifest, ...parsed };
        content = JSON.stringify(withMeta, null, 2);
        break;
      }
      case 'brd': {
        content = generateBRD(project, i18nLang);
        content += `\n<!-- Generated by Ontology Architect v${manifest.generator.version} -->`;
        break;
      }
      case 'integration': {
        content = generateIntegrationGuide(project, i18nLang);
        content += `\n<!-- Generated by Ontology Architect v${manifest.generator.version} -->`;
        break;
      }
    }
  ...
  const handleDownloadZip = async () => {
    ...
    const qualityReport = runQualityCheck(project);
    const manifest = await buildManifest(project, 'zip', qualityReport.score);
